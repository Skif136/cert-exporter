...
function checkCert () {
  ...
  echo -n | openssl s_client -servername "${DOMAIN}" -connect "${DOMAIN}" 2>/dev/null | sed -ne '/-BEGIN CERTIFICATE-/,/-END CERTIFICATE-/p' > $certificate_file
  # get expiry date
  ...
  echo "cert_status{endpoint: adress:\"${DOMAIN}\"} $date_diff"
